...
@lru_cache(1000)
def get_all_combs(num_colours, num_cells):
    if num_cells == 1:
...
class Game:
    ...
    def __init__(self, hidden_choices, num_colours, num_cells):
        self.hidden_choices = tuple(hidden_choices)
        self.prev_guesses = []
        self.remaining_possible = random.sample([tuple(x) for x in get_all_combs(num_colours,num_cells)],num_colours**num_cells)
...
def find_best_guess(all_moves, remaining_valid):
    """
    ...
    return minimax_val, minimax_choice 

def solve_game(hidden_position, num_colours, num_cells):
    """
    Helper method - test how long solver takes, and what it's doing
    """
    g = Game(hidden_position,num_colours,num_cells)
    guess_count, remain_possible= 0, 1e10
    logging.info(f"hidden position {hidden_position}")
    while len(g.remaining_possible) > 1:
        recommendation,_ = find_best_guess(g.all_guesses, g.remaining_possible)
...
NUM_COLOURS, NUM_CELLS = 6,3
if __name__ == "__main__":
    start = datetime.datetime.now()
    solve_guesses = 0
    for _ in tqdm(range(5)):
        pos = tuple(random.sample(range(0, NUM_COLOURS), NUM_CELLS))
        solve_guesses += solve_game(pos, NUM_COLOURS, NUM_CELLS)
    print(f"guesses {solve_guesses/ 50}")
